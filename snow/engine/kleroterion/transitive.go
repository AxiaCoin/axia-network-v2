...
import (
	...
	"github.com/axiacoin/axia-network-v2/snow/consensus/kleroterion/poll"
	"github.com/axiacoin/axia-network-v2/snow/engine/common"
	"github.com/axiacoin/axia-network-v2/snow/events"
...
type Transitive struct {
	...
	// Block ID --> Block
	pending map[ids.ID]kleroterion.Block
...
func newTransitive(config Config) (*Transitive, error) {
	...
	t := &Transitive{
		...
		pending:                 make(map[ids.ID]kleroterion.Block),
...
func (t *Transitive) Notify(msg common.Message) error {
	t.Ctx.Log.Verbo("kleroterion engine notified of %s from the vm", msg)
	switch msg {
...
func (t *Transitive) Start(startReqID uint32) error {
	...
	if oracleBlk, ok := lastAccepted.(kleroterion.OracleBlock); ok {
		options, err := oracleBlk.Options()
		switch {
		case err == kleroterion.ErrNotOracle:
...
func (t *Transitive) GetBlock(blkID ids.ID) (kleroterion.Block, error) {
...
func (t *Transitive) issueFrom(vdr ids.ShortID, blk kleroterion.Block) (bool, error) {
	blkID := blk.ID()
...
func (t *Transitive) issueWithAncestors(blk kleroterion.Block) (bool, error) {
	blkID := blk.ID()
...
func (t *Transitive) wasIssued(blk kleroterion.Block) bool {
	blkID := blk.ID()
...
func (t *Transitive) issue(blk kleroterion.Block) error {
	blkID := blk.ID()
...
func (t *Transitive) pushQuery(blk kleroterion.Block) {
	t.Ctx.Log.Verbo("about to sample from: %s", t.Validators)
	vdrs, err := t.Validators.Sample(t.Params.K)
	if err != nil {
...
func (t *Transitive) deliver(blk kleroterion.Block) error {
	blkID := blk.ID()
	...
	added := []kleroterion.Block{}
	dropped := []kleroterion.Block{}
	if blk, ok := blk.(kleroterion.OracleBlock); ok {
...
func (t *Transitive) removeFromPending(blk kleroterion.Block) {
	delete(t.pending, blk.ID())
}

func (t *Transitive) addToNonVerifieds(blk kleroterion.Block) {
