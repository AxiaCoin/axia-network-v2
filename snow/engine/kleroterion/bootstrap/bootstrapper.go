...
func (b *bootstrapper) Ancestors(vdr ids.ShortID, requestID uint32, blks [][]byte) error {
	...
	blockSet := make(map[ids.ID]kleroterion.Block, len(blocks))
...
func (b *bootstrapper) ForceAccepted(acceptedContainerIDs []ids.ID) error {
	...
	toProcess := make([]kleroterion.Block, 0, len(acceptedContainerIDs))
...
// process a block
func (b *bootstrapper) process(blk kleroterion.Block, processingBlocks map[ids.ID]kleroterion.Block) error {
	status := blk.Status()
	blkID := blk.ID()
