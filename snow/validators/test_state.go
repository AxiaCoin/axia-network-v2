...
type TestState struct {
	...
	GetValidatorSetF  func(height uint64, allychainID ids.ID) (map[ids.ShortID]uint64, error)
}
...
func (vm *TestState) GetValidatorSet(height uint64, allychainID ids.ID) (map[ids.ShortID]uint64, error) {
	if vm.GetValidatorSetF != nil {
		return vm.GetValidatorSetF(height, allychainID)
	}
