...
import (
	...
	"github.com/axiacoin/axia-network-v2/snow/consensus/metrics"
...
// Topological implements the Kleroterion interface by using a tree tracking the
...
type Topological struct {
	...
	// ctx is the context this kleroterion instance is executing in
	...
	blocks map[ids.ID]*kleroterionBlock // blockID -> kleroterionBlock
...
func (ts *Topological) Initialize(ctx *snow.ConsensusContext, params snowball.Parameters, rootID ids.ID, rootHeight uint64) error {
	...
	ts.blocks = map[ids.ID]*kleroterionBlock{
		rootID: {sm: ts},
	}
	ts.tail = rootID
...
func (ts *Topological) Add(blk Block) error {
	...
	ts.blocks[blkID] = &kleroterionBlock{
		sm:  ts,
		blk: blk,
	}
...
func (ts *Topological) HealthCheck() (interface{}, error) {
	...
	if !healthy {
		...
		return details, fmt.Errorf("kleroterion consensus is not healthy reason: %s", strings.Join(errorReasons, ", "))
	}
	return details, nil
}
...
// accept the preferred child of the provided kleroterion block. By accepting the
...
func (ts *Topological) accept(n *kleroterionBlock) error {
