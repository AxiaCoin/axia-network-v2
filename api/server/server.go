...
type Server interface {
	...
	// holds the engine's context lock. Namely, this could happen when the CoreChain is
	// creating a new chain and holds the CoreChain's lock when this function is held,
	// and at the same time the server's lock is held due to an API call and is trying
	// to grab the CoreChain's lock.
