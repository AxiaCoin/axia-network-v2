...
import (
	...
	"github.com/axiacoin/axia-network-v2/vms/proposervm/scheduler"
	"github.com/axiacoin/axia-network-v2/vms/proposervm/state"
	"github.com/axiacoin/axia-network-v2/vms/proposervm/tree"
...
func (vm *VM) Initialize(
	...
	vm.Windower = proposer.New(ctx.ValidatorState, ctx.AllychainID, ctx.ChainID)
	vm.Tree = tree.New()
...
func (vm *VM) SetPreference(preferred ids.ID) error {
	...
	if err != nil {
		...
		// Corechain height. This will cause building blocks to return an error
		// until the Corechain's height has advanced.
		return nil
	}
