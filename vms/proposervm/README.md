...
- `PChainHeight` the height of the last accepted block on the Corechain at the time the block is produced.
...
For a given block, Snowman++ randomly selects a list of proposers. Block proposers are selected from the subnet's validators. Snowman++ extracts the list of a given subnet's validators from the CoreChain. Let a block have a height `H` and CoreChain height `P` recorded in its header. The proposers list for next block is generated independently but reproducibly by each node as follows:

- Subnet validators active at block `P` are retrieved from Corechain.
...
- A block must have a `PChainHeight` that is less or equal to current CoreChain height.
