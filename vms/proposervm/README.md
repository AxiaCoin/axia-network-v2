# Kleroterion++: congestion control for Kleroterion VMs

Kleroterion++ is a congestion control mechanism available for kleroterion VMs. Kleroterion++ can be activated on any kleroterion VM with no modifications to the VM internals. It is sufficient to wrap the target VM with a wrapper VM called `proposerVM` and to specify an activation time for the congestion-control mechanism. In this document we describe the high level features of Kleroterion++ and the implementation details of the `proposerVM`.

## Congestion control mechanism description

Kleroterion++ introduces a **_soft proposer mechanism_** which attempts to select a single proposer with the power to issue a block, but opens up block production to every validator if sufficient time has passed without blocks being generated.

At a high level, Kleroterion++ works as follows: for each block a small list of validators is randomly sampled, which will act as "proposers" for the next block. Each proposer is assigned a submission window: a proposer cannot submit its block before its submission window starts (the block would be deemed invalid), but it can submit its block after its submission window expires, competing with next proposers. If no block is produced by the proposers in their submission windows, any validator will be free to propose a block, as happens for ordinary kleroterion VMs.

In the following we detail the block extensions, the proposers selection, and the validations introduced for Kleroterion++.

### Kleroterion++ block extension

Kleroterion++ does not modify the blocks produced by the VM it is applied to. It extends these blocks with a header carrying the information needed to control block congestion.
...
- `CoreChainHeight` the height of the last accepted block on the Core-chain at the time the block is produced.
...
### Kleroterion++ proposers selection mechanism

For a given block, Kleroterion++ randomly selects a list of proposers. Block proposers are selected from the allychain's validators. Kleroterion++ extracts the list of a given allychain's validators from the Core-Chain. Let a block have a height `H` and Core-Chain height `P` recorded in its header. The proposers list for next block is generated independently but reproducibly by each node as follows:

- Allychain validators active at block `P` are retrieved from Core-chain.
...
- A block must have a `CoreChainHeight` that is less or equal to current Core-Chain height.
...
Kleroterion++ must have an activation time, following which the congestion control mechanism will be enforced.
