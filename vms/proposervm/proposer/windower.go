...
// windower interfaces with CoreChain and it is responsible for calculating the
...
func (w *windower) Delay(chainHeight, pChainHeight uint64, validatorID ids.NodeID) (time.Duration, error) {
	...
	// get the validator set by the corechain height
	validatorsMap, err := w.state.GetValidatorSet(pChainHeight, w.allychainID)
	if err != nil {
		return 0, err
	}
