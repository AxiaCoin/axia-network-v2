...
## [v1.7.12](https://github.com/axiacoin/axia-network-v2/releases/tag/v1.7.12)
...
- Improved Axchain state sync leaf request serving by optimistically reading leaves from snapshot
- Refactored Axchain state sync block fetching
...
- Improved Axchain state sync metrics

### Performance

- Added Axchain acceptor queue within `core/blockchain.go`
- Removed rpcdb locking when committing batches and using iterators
- Capped Axchain TrieDB dirties cache size during block acceptance to reduce commit size at 4096 block interval
...
## [v1.7.11](https://github.com/axiacoin/axia-network-v2/releases/tag/v1.7.11)

This version is backwards compatible to [v1.7.0](https://github.com/axiacoin/axia-network-v2/releases/tag/v1.7.0). It is optional, but encouraged.

**The first startup of the AXChain will cause an increase in CPU and IO usage due to an index update. This index update runs in the background and does not impact restart time.**
...
- Fixed panic on concurrent map read/write in Corechain axiawallet SDK
...
- Added a basic Swapchain E2E usage test to the new testing framework.
...
## [v1.7.6](https://github.com/axiacoin/axia-network-v2/releases/tag/v1.7.6)
...
- Increased `FeeHistory` maximum historical limit to improve MetaMask UI on the AXChain.
...
- Implemented Swapchain + Corechain axiawallet that can be used to build and sign transactions. Without providing a full node private keys.
...
**The first startup of the AXChain will take a few minutes longer due to an index update.**
...
- Introduced Whitelist conflicts into the Snowstorm specification that will be used in future Swapchain improvements.
- Refactored the separation between the Bootstrapping engine and the Consensus engine to support Fast-Sync.

### Coreth

- Added an index mapping height to the list of accepted atomic operations at that height in a trie. Generating this index will cause the node to take a few minutes longer to startup the AXChain for the first restart.
...
- Introduced a notion of vertex conflicts that will be used in future Swapchain improvements.

### Coreth

- Added an index mapping height to the list of accepted atomic transactions at that height. Generating this index will cause the node to take approximately 2 minutes longer to startup the AXChain for the first restart.
...
This upgrade adds support for issuing multiple atomic transactions into a single block and directly transferring assets between the Corechain and the Axchain.
...
- Added the ability to export/import AXC to/from the Axchain.
