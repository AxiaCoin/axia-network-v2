...
import (
	...
	"github.com/axiacoin/axia-network-v2/vms/avm/fxs"
...
var (
	...
	errNoAXChainGenesis        = errors.New("AXChain genesis cannot be empty")
...
func validateConfig(networkID uint32, config *Config) error {
	...
	if len(config.AXChainGenesis) == 0 {
		return errNoAXChainGenesis
	}
...
func FromConfig(config *Config) ([]byte, ids.ID, error) {
	...
	}
	{
		...
		for _, allocation := range xAllocations {
			addr, err := address.FormatBech32(hrp, allocation.AXCAddr.Bytes())
			if err != nil {
	...
	genesisStr, err := formatting.EncodeWithChecksum(defaultEncoding, []byte(config.AXChainGenesis))
	if err != nil {
		return nil, ids.Empty, fmt.Errorf("couldn't encode message: %w", err)
	}
...
func splitAllocations(allocations []Allocation, numSplits int) [][]Allocation {
	...
	for _, allocation := range allocations {
		...
		// Already added to the Swapchain
