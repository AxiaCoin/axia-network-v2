...
rm -rf /var/lib/axiago 2>/dev/null || true # Do || true to ignore error if files dont exist yet
...
aws s3 cp s3://axia-db-daily/ /opt/ --no-progress --recursive --exclude "*" --include "$DB_FILE*" 
echo "Done downloading database"

# extract DB
echo "Extracting database..."
mkdir -p /var/lib/axiago/db 
tar -zxf /opt/$DB_FILE*-tar.gz -C /var/lib/axiago/db 
...
containerID=$(docker run --name="net_outage_simulation" --memory="12g" --memory-reservation="11g" --cpus="6.0" --net=controlled-net -p 9650:9650 -p 9651:9651 -v /var/lib/axiago/db:/db -d axiaplatform/axiago:latest /axiago/build/axiago --db-dir /db --http-host=0.0.0.0)
