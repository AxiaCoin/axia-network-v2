...
rm -rf /var/lib/axia 2>/dev/null || true # Do || true to ignore error if files dont exist yet
...
mkdir -p /var/lib/axia/db 
tar -zxf /opt/$DB_FILE*-tar.gz -C /var/lib/axia/db 
...
containerID=$(docker run --name="net_outage_simulation" --memory="12g" --memory-reservation="11g" --cpus="6.0" --net=controlled-net -p 9650:9650 -p 9651:9651 -v /var/lib/axia/db:/db -d axiaplatform/axia:latest /axia/build/axia --db-dir /db --http-host=0.0.0.0)
